...
    '=========================
    'This macro calculates future dates based on user-selected text representing a date in a valid format.
    'Note that ordinal indicators (e.g., "st", "nd", "rd", etc.) are not considered valid.
    'The user is prompted to enter the number of days to add to the selected date. Results are displayed
    'in a message box. The output includes the original date, the date after adding the specified days,
    'and the date including one additional day (end date inclusive), all formatted as "dd-mmm-yyyy".
